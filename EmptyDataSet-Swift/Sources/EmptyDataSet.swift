...
extension UIScrollView: UIGestureRecognizerDelegate {
    ...
    private var userSectionCountForDisplay: Bool {
        return emptyDataSetSource?.shouldUseSectionCountForDisplay() ?? false
    }
            ...
            if let view = emptyDataSetView {
                ...
                if let customView = customView {
                    ...
                    if let buttonImage = buttonImage(for: .normal) {
                        ...
                        view.button.setBackgroundImage(buttonBackgroundImage(for: .normal), for: .normal)
                        view.button.setBackgroundImage(buttonBackgroundImage(for: .highlighted), for: .highlighted)
                    }
                }
